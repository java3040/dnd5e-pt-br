...
export class Converters {
  ...
  static _imperialToMetric(value, translation, data) {
    ...
    if (conversion) {
      // for (const key of ["bright", "burrow", "climb", "dim", "distance", "fly", "long", "paces", "range", "ranges", "reach", "speeds", "swim", "template", "units", "value", "walk"]) {
      //   switch (key) {
      //     case "bright":
      //     case "burrow":
      //     case "climb":
      ...
      //     case "value":
      //     case "walk":
      //       if (value?.[key]) converted[key] = conversion.converter(value[key]);
      //       break;
      //     case "units":
      //       if (value?.[key]) converted[key] = conversion.units;
      //       break;
      //     case "template":
      //       if (value?.[key])
      //         converted[key] = {
      //           ...Object.fromEntries(["size", "height", "width"].map((k) => [k, conversion.converter(value.template[k])])),
      //           units: conversion.units,
      //         };
      //       break;
      //     case "ranges":
      //       if (value?.[key]) converted[key] = Object.fromEntries(["darkvision", "blindsight", "tremorsense", "truesight"].map((k) => [k, conversion.converter(value.ranges[k])]));
      //       break;
      //     case "paces":
      //       if (value?.[key]) converted[key] = Object.fromEntries(["air", "land", "water"].map((k) => [k, conversion.converter(value.paces[k])]));
      //       break;
      //     case "speeds":
      //       if (value?.[key]) converted[key] = Object.fromEntries(["air", "land", "water"].map((k) => [k, conversion.converter(value.speeds[k])]));
      //       break;
      //     default:
      //       break;
      //   }
      ...
      if (value?.ranges) {
        converted.ranges = Object.fromEntries(["darkvision", "blindsight", "tremorsense", "truesight"].map((k) => [k, conversion.converter(value.ranges[k])]));
      }
      if (value?.paces) {
        converted.paces = Object.fromEntries(["air", "land", "water"].map((k) => [k, conversion.converter(value.paces[k])]));
      }
      if (value?.speeds) {
        converted.speeds = Object.fromEntries(["air", "land", "water"].map((k) => [k, conversion.converter(value.speeds[k])]));
      }
      ...
      if (Object.keys(value ?? {}).some((k) => detectionModeKeys.has(k))) {
        return Object.fromEntries(Object.entries(value).map(([k, mode]) => [k, mode?.range ? { ...mode, range: conversion.converter(mode.range) } : mode]));
      }

      if (value && typeof value === "object" && !Array.isArray(value) && Object.values(value).every((v) => v && typeof v === "object" && "value" in v && Object.keys(v).length === 1)) {
        return Object.fromEntries(Object.entries(value).map(([k, entry]) => [k, { value: conversion.converter(entry.value) }]));
      }
    }
  ...
  static get conversionInfo() {
    return {
      ...
      mph: { converter: Converters.milesToMeters, units: convertMetricLength() ? "km/h" : "mph" },
      lb: { converter: Converters.lbToKg, units: convertMetricWeight() ? "kg" : "lb" },
      cubicFoot: { converter: Converters.pcToL, units: convertMetricVolume() ? "L" : "cubicFoot" },
    };
  }
  ...
  static _tokens(tokens, translations, data, tc, runtime = {}) {
    ...
    const documentMappings = runtime?.currentCompendium?.()?.documentMappings ?? game.babele.documentMappings;
    ...
    return tokens.map((token) => {
      ...
      deltaTranslated = foundry.utils.mergeObject(foundry.utils.deepClone(delta), payload, { inplace: false });

      return foundry.utils.mergeObject(token, { name: translation.name ?? token.name, delta: deltaTranslated }, { inplace: false });
    });
  }
}
