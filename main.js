...
export class Converters {
  static imperialToMetric(type) {
    return (value) => {
      switch (type) {
        ...
        case "volume":
          return Converters.volume(value);
        case "travel":
          return Converters.travel(value);
        case "movement":
          return Converters.movement(value);
        case "tokenLight":
          return Converters.tokenLight(value);
        case "sightRange":
          return Converters.footsToMeters(value);
        case "communication":
          return Converters.communication(value);
  ...
  static distanceAdvancement(advancements) {
    Object.keys(advancements).forEach((key) => {
      ...
      if (adv.type === "ScaleValue" && adv.configuration.type === "distance") {
        const conversion =
          Converters.conversionInfo[adv.configuration.distance.units || "ft"];
        if (conversion) {
  ...
  static pages() {
    return (pages, translations, data) =>
      Converters._pages(pages, translations, data);
  }
      ...
      if (Array.isArray(journal)) {
        ...
        if (journal.conserveOriginalSort) {
          ...
          pages = pages
            ...
            .sort((a, b) => {
              const nameA = journal.includeArticle
                ? a.name
                : normalizeName(a.name);
              const nameB = journal.includeArticle
                ? b.name
        ...
        if (journal.pagesGroups?.length) {
          ...
          for (const group of journal.pagesGroups) {
            const parentIndex = workingPages.findIndex(
              (p) => p._id === group.parent,
            );
            ...
            if (group.isRange && group.associatePages.length === 2) {
              ...
              const startIndex = workingPages.findIndex(
                (p) => p._id === startId,
              );
            ...
            const before = workingPages
              ...
              .filter(
                (p) =>
                  !firstPagesIds.includes(p._id) && !groupIds.includes(p._id),
              );
            ...
            if (group.isRange) {
              sortedGroup.sort((a, b) => {
                const nameA = group.includeArticle
                  ? a.name
                  : normalizeName(a.name);
                const nameB = group.includeArticle
                  ? b.name
                  : normalizeName(b.name);
                return nameA.localeCompare(nameB);
              });
  ...
  static tokens() {
    return (tokens, translations, data, tc) =>
      Converters._tokens(tokens, translations, data, tc);
  }
  ...
  static _tokens(tokens, translations, data, tc) {
    ...
    return tokens.map((token) => {
      ...
      const actorConverter = babeleConverters.fromDefaultMapping(
        "Actor",
        "actors",
      );
      const deltaTranslated = delta
        ? actorConverter([delta], { [delta._id]: translation }, data, tc)[0]
        : delta;
      if (!deltaTranslated.name)
        deltaTranslated.name = translation.name ?? delta.name;
      return foundry.utils.mergeObject(token, {
  ...
  static alignment() {
    return (alignment, translation, data) =>
      Converters._alignment(alignment, translation, data);
  }
  ...
  static _alignment(alignment, translation, data) {
    ...
    const alignments = {
      good: { f: "Boa", m: "Bom" },
      evil: { f: "Má", m: "Mau" },
      neutral: { f: "Neutra", m: "Neutro" },
      lawful: { f: "Leal", m: "Leal" },
      chaotic: { f: "Caótica", m: "Caótico" },
      unaligned: { f: "sem alinhamento", m: "sem alinhamento" },
    };
    ...
    const translated = alignment
      ...
      .map((part) => {
        ...
        if (key === "typically") return "normalmente";
  ...
  static size() {
    return (size, translation, data) =>
      Converters._size(size, translation, data);
  }

  static _size(size, translation, data) {
    if (translation) return translation;

    const sizes = {
      tiny: { f: "Minúscula", m: "Minúsculo" },
      sm: { f: "Pequena", m: "Pequeno" },
      med: { f: "Média", m: "Médio" },
      lg: { f: "Grande", m: "Grande" },
      huge: { f: "Enorme", m: "Enorme" },
      grg: { f: "Colossal", m: "Colossal" },
    };
    ...
    const type = data?.system?.details?.type?.value?.toLowerCase();
    const gender = monsterTypes[type] ?? "m"; // Padrão masculino se não encontrar
    ...
    Object.keys(activities).forEach((key) => {
      ...
      if (translation) {
        foundry.utils.mergeObject(activity, {
          ...
          duration: {
            special: translation.duration ?? activity.duration?.special,
          },
          range: { special: translation.range ?? activity.range?.special },
          target: {
            affects: {
              special: translation.target ?? activity.target?.affects?.special,
            },
          },
    ...
    Object.keys(advancements).forEach((key) => {
      ...
      if (translation) {
        foundry.utils.mergeObject(adv, {
          ...
          configuration: {
            identifier:
              adv.configuration.identifier?.length > 0
  ...
  static planarSubtype() {
    return (habitat, translation) =>
      Converters._planarSubtype(habitat, translation);
  }
  ...
  static _planarSubtype(habitat, translation) {
    ...
    return habitat.map((hab) => {
      if (hab.subtype)
        return foundry.utils.mergeObject(hab, {
          subtype: translation ?? hab.subtype,
        });
      return hab;
    });
  }
